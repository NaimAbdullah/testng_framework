...
//        actions.dragAndDrop(dragMeBox, dropHereBox).perform();
    ...
    @Test(priority = 6, description = "Action class | Send Keys to an Element")
    ...
    public void etsyTest(){
        ...
        String[] texts = {"Accessories", "Bags & Purses", "Necklaces", "Rings",
                "Earrings", "Bracelets", "Body Jewelry", "All Jewelry"};

        for (int i = 0; i < 8; i++) {
            Waiter.waitUntilTextToBePresentInElement(driver, 10, etsySearchPage.jeweleryAndAccessoriesItems.get(i), texts[i]);
            Assert.assertEquals(etsySearchPage.jeweleryAndAccessoriesItems.get(i).getText(), texts[i]);
        }
    }
}
